import{i as d,F as h,b as n,a as u,_ as o,t as p}from"./fetch-retrier-DQgSUyIF.js";import{b as a}from"../app-root.js";import{r as i}from"./state-iAArRRY0.js";const f={default:n.default,noRetry:n.noRetry};class m{constructor(t,s){this.failCount=t,this.onAttempt=s,this.callCount=0}install(){this.original=window.fetch.bind(window),window.fetch=async()=>{this.callCount+=1;const t=this.callCount<=this.failCount?503:200;return this.onAttempt(t),new Response(JSON.stringify({attempt:this.callCount,ok:t===200}),{status:t})}}uninstall(){this.original&&(window.fetch=this.original)}}let e=class extends d{constructor(){super(...arguments),this.configKey="default",this.failCount=2,this.loading=!1,this.log=[]}async sendRequest(){this.log=[],this.outcome=void 0,this.loading=!0;const t=performance.now(),s=new m(this.failCount,l=>{this.log=[...this.log,{attempt:this.log.length+1,status:l,timeSinceStartMs:Math.round(performance.now()-t)}]});s.install();try{const c=await new h({retryConfig:f[this.configKey]}).fetchRetry("/demo/flaky-endpoint");this.outcome={success:c.ok,totalMs:Math.round(performance.now()-t)}}finally{s.uninstall(),this.loading=!1}}render(){return a`
